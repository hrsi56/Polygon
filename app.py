...
def draw_polygon_fig(poly: PolygonData, show_alt: bool):
    ...
    for d in diags:
        ...
        ax.text(*(p1 + vec_i * 0.1 * min_len),
                f"{d['end_i']['angle']:.1f}°\n{d['end_i']['side']}",
                fontsize=5, color='brown', ha='center', va='center')
...
app.layout = html.Div([
    ...
    html.H2('לינקו - תמורת טובות הנאה.'),
...
def update_length_inputs(n):
    return [
        html.Div([
            ...
            dcc.Input(id={'type': 'length', 'index': i}, type='number', min=0.1, max=10000, step=0.5, value=1.0)
        ]) for i in range(n)
    ]
...
def update_angle_inputs(show, n):
    if 'show' in show:
        return [
            html.Div([
                html.Label(f"Angle {vertex_names(n)[i]}"),
                dcc.Input(id={'type': 'angle', 'index': i}, type='number', min=1, max=360, step=1, value=round(180 * (n - 2) / n, 1))
            ]) for i in range(n)
        ]
    return []
