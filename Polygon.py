...
def draw_polygon(poly: PolygonData, show_altitudes: bool):
    ...
    def draw_angle_text(ax, origin, vec1, vec2, label, radius, color="red", fontsize=7):
        """
        מציב תווית זווית בין שני וקטורים היוצאים מ־origin
        """
        u = vec1 / np.linalg.norm(vec1)
        v = vec2 / np.linalg.norm(vec2)
        bis = u + v
        if np.linalg.norm(bis) < 1e-8:
            bis = np.array([-u[1], u[0]])  # מקרה של 180°
        else:
            bis /= np.linalg.norm(bis)
        pos = origin + bis * radius
        ax.text(*pos, label, color=color, fontsize=fontsize,
                ha="center", va="center")
    ...
    for i in range(n):
        ...
        draw_angle_text(ax, p1, edge_vec, best_vec, f"{best_angle:.1f}°", radius=0.2 * min_len)
